...
def points2grid(points, resolution=params['resolution'], depth=params['depth']):
    ...
    Args:
        points (torch.tensor): of size [B, _, 6]
    Returns:
        grid (torch.tensor): of size [B * self.num_views, depth, resolution, resolution]
    """
    points_xyz = points[:, :, :3]
    batch, pnum, _ = points_xyz.shape

    pmax, pmin = points_xyz.max(dim=1)[0], points_xyz.min(dim=1)[0]
    pcent = (pmax + pmin) / 2
    pcent = pcent[:, None, :]
    prange = (pmax - pmin).max(dim=-1)[0][:, None, None]
    points_xyz = (points_xyz - pcent) / prange * 2.
    points_xyz[:, :, :2] = points[:, :, :2] * params['obj_ratio']
    
    depth_bias = params['depth_bias']
    _x = (points_xyz[:, :, 0] + 1) / 2 * resolution
    _y = (points_xyz[:, :, 1] + 1) / 2 * resolution
    _z = ((points_xyz[:, :, 2] + 1) / 2 + depth_bias) / (1+depth_bias) * (depth - 2)
...
class Realistic_Projection:
    ...
    def point_transform(points, rot_mat, rot_mat2, translation):
        ...
        points_xyz = points[:, :, :3]
        points_xyz = torch.matmul(points_xyz, rot_mat)
        points_xyz = torch.matmul(points_xyz, rot_mat2)
        points_xyz = points_xyz - translation
        points = torch.cat((points_xyz, points[:, :, 3:]), dim=2)
        return points
